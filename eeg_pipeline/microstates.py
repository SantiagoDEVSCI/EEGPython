...
def evaluate_k_range(maps: np.ndarray, data: np.ndarray, k_min: int, k_max: int,
                     n_init: int, max_iter: int, ignore_polarity: bool=True, random_state: int=42):
    ks, gevs = [], []
    for k in range(k_min, k_max+1):
        centers, _ = kmeans_maps(maps, k, n_init, max_iter, random_state)
        _, gev = backfit(centers, data, ignore_polarity)
        ks.append(k); gevs.append(gev)
    return np.array(ks), np.array(gevs)

def metrics_from_labels(labels: np.ndarray, sfreq: float, k: int) -> dict:
    cov = np.zeros(k); dur = np.zeros(k); occ = np.zeros(k)
    n = len(labels)
    for s in range(k):
        mask = labels == s
        cov[s] = mask.mean()
